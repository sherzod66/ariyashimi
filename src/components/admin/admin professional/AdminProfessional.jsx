import styles from './adminProfessional.module.css'
import { IoIosAdd } from 'react-icons/io'
import { AiOutlineSearch } from 'react-icons/ai'
import { useEffect, useState } from 'react'
import AdminProfessionalPopap from './AdminProfessionalPopap'
...
const AdminProfessional = () => {
	...
	useEffect(() => {
		ReadNews(setNews, 'professional')
	}, [])
	const [popap, setPopap] = useState(false)
	return <main className={styles.product}>
		{popap ? <AdminProfessionalPopap state={setPopap} /> : ""}
		{detail.show ? <AdminFeedbackPopup active={detail.data} set={setDetail} isExist={true} /> : ''}
		<div className={styles.product__body}>
			<div className={styles.product__navigate_bar}>
				...
				<div onClick={() => setPopap(!popap)} className={styles.product__addNew}><IoIosAdd className={styles.plus} />Add people from the technical department</div>
			</div>
			{newsData.length > 0 ? newsData.map(el => (
				<div key={el.id} className={styles.adminFeedback__column}>
					<div className={styles.adminFeedback__first_name}><p>{el.Ename}</p></div>
					...
					<div onClick={(e) => DeleteDocument('professional', el.id, navigate)} className={styles.adminFeedback__delete}>Delete</div>
