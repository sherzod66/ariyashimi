...
import styles from './adminProfessionalPopap.module.css'
...
const AdminProfessionalPopap = ({ state }) => {
	...
	const addExpert = (e) => {
		e.preventDefault()
		if (isExists)
			AddExpert(salesman, setSalesman, 'professional', setDie, navigate)
		else
	...
	return <div onClick={(e) => !e.target.closest('#popup_body') ? state(false) : ''} className={styles.popup}>
		<div id='popup_body' className={styles.popup__body}>
			...
			<p>Add a professional from the technical department</p>
...
export default AdminProfessionalPopap